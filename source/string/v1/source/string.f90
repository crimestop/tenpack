...
integer,parameter::max_char_length=200
...
function dbl2str(num,length) result(str2)
	...
	if(abs_num>compare(size(compare))*(1 - 5d-7) .or. abs_num<compare(1)*(1 - 5d-7))then
		tenexp=0
		if(abs_num>1 - 5d-7)then
			do while(abs_num>=10)
				tenexp=tenexp+1
				abs_num=abs_num/10
			end do
		else if (abs_num<1 - 5d-7)then
			do while(abs_num<1)
				tenexp=tenexp-1
				abs_num=abs_num*10
			end do
		end if
		!now 1 - 5d-7<=abs_num<10 - 5d-6
		...
		dotpos=count(abs_num>(compare*(1 - 5d-7)))
...
function int_ary2str(nums) result(str2)
...
function flt_ary2str(nums) result(str2)
...
function dbl_ary2str(nums) result(str2)
