...
type unidic
	...
	integer,public::item_num=0,max_val=0,stack_num=0
...
implicit none
	...
	if(.not.found) call wc_error_stop('unidic.rename','key1 '//trim(key1)//' not exist')

	pos=hash_func(U,key2)

	if(.not.associated(U%hash_ary(pos)%first))then
		allocate(U%hash_ary(pos)%first)
		U%hash_ary(pos)%first%key=key2
		U%hash_ary(pos)%first%val=val
	else
		p=>U%hash_ary(pos)%first
		if(p%key==key2)then
			call wc_error_stop('unidic.rename','key2 '//trim(key2)//' already exist')
		end if
		do while(associated(p%next))
			p=>p%next
			if(p%key==key2)then
