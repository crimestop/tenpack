...
contains
	subroutine set_error_backtrace()
		...
		call writemess(' The error_backtrace can print the location where the bugs occus!')
		call writemess(' If you Running on Mac, this message is in ')
		call writemess('   $HOME/Library/Logs/DiagnosticReports!')
	end subroutine
...
!	character(len=(max(len(w1),max_len_of_char))) function intAddchar(inte,w1)
	...
	subroutine open_File(uni,FileAddress,inSTATUS)
	...
	logical function ifopen_File(uni,FileAddress,inSTATUS)result(alive)
		character(len=*),intent(in)::FileAddress
		character(len=*),intent(in)::inSTATUS
		integer,intent(in)::uni
		if(inSTATUS.equ.'old')then
