...
!			norm(Tr):return  sqrt(<phi|phi>)
		...
		procedure::SVDTensor_Leg1_Routine,SVDTensor_Leg2_Routine,SVDTensor_Leg1_Routine_
...
!			 T=res(1)*res(res(2))*res(3)
		...
		generic,public::QRTensor =>QRTensor_noName,QRTensor_Name,QRTensor_Leg1,QRTensor_Leg2
		generic,public::QRroutine     => QRdecomposition1,QRTensor_Leg1_Routine,QRTensor_Leg2_Routine
		procedure::QRTensor_Leg1_Routine,QRTensor_Leg2_Routine,QRdecomposition1
		procedure::QRTensor_Leg1,QRTensor_Leg2
! 			T should be two dimension		
!			T=Q*R
!			 on output,the order in the array is
! 			[Q]->[R]
!			 T=res(1)*res(2)		
		...
		generic,public::LQTensor =>LQTensor_noName,LQTensor_Name,LQTensor_Leg1,LQTensor_Leg2
		generic,public::LQroutine     => LQdecomposition1,LQTensor_Leg2_Routine,LQTensor_Leg1_Routine
		procedure::LQTensor_Leg2_Routine,LQTensor_Leg2,LQTensor_Leg1_Routine,LQTensor_Leg1,LQdecomposition1
! 			T should be two dimension		
!			T=L*Q
!			 on output,the order in the array is
! 			[L]->[Q]
...
!
!**********************************************************	
	public::Working_memory_report,Tensor_memory_report,Tensor_memory_length
contains
...
!	allocatedTensor will not change the Dynamic of Tensor
	subroutine allocatedTensor1(T,dimen,typede)
		...
		length=dimen%size()
		if(length.le.0) then
			write(*,*)"ERROR in allocatedTensor"
			call dimen%print()
			call error_stop()
		end if
...
!                  store data      in Tensor, the input data can be any chape   
	...
	subroutine modify_Some_Data1_char(A,ia,B,ib)	
		...
		call modify_chech_inputParameter(ib,lenB)
		call modify_chech_inputindex(ia,ib)
		call modify_Some_TData_char1(A%TData,A%GetTotalData(),ia,B,lenB,ib)
		return
	end subroutine
	...
	subroutine modify_Some_Data2_char(A,ia,ja,B,ib,jb)	
		...
		call modify_chech_inputindex(ia,ib)
		call modify_chech_inputindex(ja,jb)
		call modify_Some_TData_char2(A%TData,Adim,ia,ja,B,Bdim,ib,jb)
		return
	end subroutine
...
!
	...
	type(Tensor) function constructor_char_scal(val,dimen)result(Res)
	...
	integer function iElement(T,Tdim)
		...
		if(T%rank.eq.1)then
			if(Tdim(1).gt.getTotalData(T))Then
				write(*,*)"Index is larger than the len of Tensor"
				call error_stop()
			end if
...
!**************************************************************************************************************
!**************************************************************************************************************
	subroutine modifyTen_val_class(Ten,dimen,val)!in will go wrong for character
	...
	subroutine modifyTen_val1_class(Ten,ith,val)
		class(Tensor),intent(inout)::Ten
		integer,intent(in)::ith
		class(*),intent(in)::val
	...
	subroutine modifyTen_val_char(Ten,dimen,val)
		...
		if(Ten%rank.eq.1) then
			...
			allocate(LDdimen(1))
			LDdimen=Ten.dim.1
			call modify_TData_char(Ten,dimen,LDdimen,1,val)
			return
		end if
		if(Ten%rank.eq.2) then
			if( (dimen(1).gt.(Ten.dim.1)) .or. (dimen(2).gt.(Ten.dim.2)))Then
			...
			allocate(LDdimen(2))
			LDdimen=.subdim.Ten
			call modify_TData_char(Ten,dimen,LDdimen,2,val)
			return
		end if
		if(Ten%rank.eq.3) then
			if( (dimen(1).gt.(Ten.dim.1)) .or. (dimen(2).gt. (Ten.dim.2)).or. (dimen(3).gt.(Ten.dim.3)))Then
			...
			allocate(LDdimen(3))
			LDdimen=.subdim.Ten
			call modify_TData_char(Ten,dimen,LDdimen,3,val)
			return
		end if
		if(Ten%rank.eq.4) then
			if( (dimen(1).gt.(Ten.dim.1)) .or. (dimen(2).gt. (Ten.dim.2)).or. (dimen(3).gt.(Ten.dim.3))&
			...
			allocate(LDdimen(4))
			LDdimen=.subdim.Ten
			call modify_TData_char(Ten,dimen,LDdimen,4,val)
			return
		end if
		addre=addressToIndes(Ten,dimen)
		if(addre.gt.getTotalData(Ten))Then
		...
		call modify_TData_char(Ten,(/addre/),(/getTotalData(Ten)/),1,val)
		return
	end subroutine
	...
	subroutine modifyTen_val1_char(Ten,ith,val)
		...
		integer,intent(in)::ith
		character(len=*),intent(in)::val
			call modify_TData_char(Ten,(/ith/),(/getTotalData(Ten)/),1,val)
		return
	end subroutine
			...
			case (0)
				if((total1.eq.1).and.(total2.eq.1)) then!there is only one element in both T1 and T2
						if((rank1.eq.1).and.(rank2.eq.1))then!Number*number,(1) *(1)
							...
							if(.not.Res%getFlag())then
								call allocateTensor(Res,T2%TenDim,classtype)
								call Res%zero()
							end if
							call product_NumNum_par(Res%TData,T1%TData,T2%TData,alpha,beta)		
							return
...
!cccccccccccccccc permutation   cccccccccccccccccc
	...
	subroutine permutation_Tensor_routine(T,Tenorder)
		...
		call WorkingMemory%check('permutation_Tensor_routine')
		select case (Tenorder%getType())
			case (1)
				!allocate(indeint(Tenorder%getTotalData()))
				call WorkingMemory%get_memory(indeint,Tenorder%getTotalData(),'permutation_Tensor_routine')
				indeint=Tenorder
				call permutation_routine(T,indeint)
			case (7)
				!allocate(indechar(Tenorder%getTotalData()))
				call WorkingMemory%get_memory(indechar,Tenorder%getTotalData(),'permutation_Tensor_routine')
				indechar=Tenorder
...
!**************************************************************************************************************
	...
	Type(Tensor) function enlargeTensorReal8(T,ithleg,newD,randomNumberScal)result(enlargeTensor)
		...
		dimen=(T.subDim.[1,ithleg-1]) 
		dimen=dimen + (T.subDim.[ithleg+1,rank]) 
		dimen=dimen+[newD]		
...
!**************************************************************************************************************	
	...
	subroutine SVDcutoff(T,U,s,V,Ncut_) 
		...
		if(associated(Tp,Up).or.associated(Tp,sp).or.associated(Tp,Vp).or.associated(Up,sp).or.&
			...
			call writemess('input Tensors can not be the same variable',-1)
			call writemess('error in call T%SVDroutine(U,S,V,Ncut)')
