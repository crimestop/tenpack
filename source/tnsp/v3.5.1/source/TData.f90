...
!**************************************************************************************************************	
	...
	subroutine SVD_TData_routine_(indata,U,S,V,m,n,min_MN_,cut,info)!There will be a bug in some case
		integer,intent(in)::min_MN_
		...
		real*8,pointer :: drw(:)
		real*4,pointer :: srw(:)
		integer :: ms_max,max_MN,length,cut2,min_MN
		integer::totallengthofmemory
		call WorkingMemory%check()
		if(present(cut))then
			min_MN=min(min_MN_,cut)
		else
			min_MN=min_MN_
		end if
