...
!***************END***********************
	...
	logical,private::error_pointer_Flag=.false.
	...
	integer,save,private::persentCharLen=50
	integer,save,private::persentCharLen2=23
!****************************************************************************
	!	MPI parameter
...
contains
	subroutine set_error_pointer()
		error_pointer_Flag=.true.
		call writemess(' Set the error_pointer')
		call writemess(' The error_pointer can print the location where the bugs occus!')
		call writemess(' This function only wrok in linux, Can not work in Mac!!')
	end subroutine
	subroutine unset_error_pointer()
		error_pointer_Flag=.false.
		call writemess(' unSet the error_pointer')
	end subroutine
	...
	subroutine error_stop()! bug , stop
		if(MPI_running)then
			...
			call writemess(.true.,'Tensor version is 3.1')
			call writemess('        ')
			call writemess('    ')
			call outpicture()
			call sleep(2)
			if(error_pointer_Flag)error_pointer=[1,2,3,4]
			!call MPI_FINALIZE( output_ierr )
			stop
		end if
		if(seed_flag)call writemess('The random seed is,seed='+initial_randomseed)
		call writemess(.true.,'Tensor version is 3.3')
