...
private
	...
	type para_list
		...
		type(tensor) :: ten_val(itemmaxn)
		character(len=max_char_length) :: ten_name(itemmaxn)=''
		character(len=max_char_length) :: ten_class(itemmaxn)=''
	...
	contains
		...
		procedure::insert_int,insert_real,insert_com,insert_char,insert_logi,insert_ten
		generic,public::insert=>insert_int,insert_real,insert_com,insert_char,insert_logi,insert_ten
...
subroutine insert_ten(D,name,value,class_)
implicit none
	...
	if(present(class_))then
		...
		if(D%itemcurn(type)==itemmaxn)then
			...
			D%ten_class(D%itemcurn(type))=class
		end if
...
implicit none
	...
	case (6)
		...
		D%ten_class(pos:itemmaxn-1)=D%ten_class(pos+1:itemmaxn)
	end select
...
subroutine tsetvalue(D,name,val)
implicit none
...
implicit none
	...
	do i=1,size(names)
		...
		case (6)
			call out_list%insert(names(i),D%ten_val(pos),D%ten_class(pos))
		end select
	end do
...
implicit none
	...
	do i=1,D%itemcurn(6)
		if(D%ten_class(i) == class) then
			call out_list%insert(D%ten_name(i),D%ten_val(i),class)
		end if
	end do
...
implicit none
	...
	do k=1,D%itemcurn(6)
		if(D%ten_class(k)==class_name)then
			formatl='t'
...
implicit none
	...
	do while(.true.)
		...
		case('t')
			...
			call tval%read(unit)
			call G%insert(cur_name,tval,cur_class)
		case('/')
			exit
		end select
...
subroutine append(P1,P2)
	...
	do k=1,P2%itemcurn(6)
		call P1%insert(P2%ten_name(k),P2%ten_val(k),P2%ten_class(k))
	end do	
