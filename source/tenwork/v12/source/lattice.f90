...
type lattice
	...
	generic,public:: get_rawpos=>get_rawpos_pos,get_rawpos_name  !only use in tn_tensor
...
subroutine set_bond_as_cluster(L,L_old,clusters)
	...
	class(lattice),intent(inout)::L
	type(lattice),intent(in)::L_old,clusters(:,:)
	character(len=max_char_length)::ind,ind2,name,name2,actual_name,actual_name2,actual_ind,actual_ind2,ind_temp,nb_ind_temp
	logical::found1,found2
	integer::i,j,m,n,p,nb_rawpos,no,nb_no,L1,L2
		...
		if(L_old%sites(i)%exist_tag)then
			do j=1,L_old%sites(i)%nb_num
				...
				if(nb_rawpos>i) then
					name=L_old%sites(i)%name
					name2=L_old%sites(nb_rawpos)%name
					ind=L_old%sites(i)%bonds(j)%ind
					nb_no=L_old%sites(i)%bonds(j)%nb_no
					ind2=L_old%sites(nb_rawpos)%bonds(nb_no)%ind
					if(L%check_exist(name))then
						found1=.true.
						actual_name=name
						actual_ind=ind
					else
						do m=1,L1
							do n=1,L2
								if(clusters(m,n)%check_exist(name))then
									actual_name=clusters(m,n)%get_name([1,1])
									if(found1) call wc_error_stop('lattice.set_bond_as','clusters overlap!')
									found1=.true.
									if(.not.clusters(m,n)%check_exist_bond(actual_name,name))then
										call wc_error_stop('lattice.set_bond_as','bond in cluster missing! from '&
											//trim(name)//' to '//trim(actual_name))
									end if
									call clusters(m,n)%get_bond(actual_name,no,ind_temp,name,nb_no,nb_ind_temp)
									actual_ind=ind_temp
								end if
							end do
						end do
						...
						do m=1,L1
							do n=1,L2
								if(clusters(m,n)%check_exist(name2))then
									...
									found2=.true.
									if(.not.clusters(m,n)%check_exist_bond(actual_name2,name2))then
										call wc_error_stop('lattice.set_bond_as','bond in cluster missing! from '&
											//trim(name2)//' to '//trim(actual_name2))
									end if
									call clusters(m,n)%get_bond(actual_name2,no,ind_temp,name2,nb_no,nb_ind_temp)
									actual_ind2=ind_temp
								end if
							end do
						end do
					end if
					if(.not. L%check_exist(actual_name))then
						call wc_error_stop('lattice.set_bond_as','cluster of '//trim(actual_name)//' missing in lattice!')
					end if
					if(.not. L%check_exist(actual_name2))then
						call wc_error_stop('lattice.set_bond_as','cluster of '//trim(actual_name2)//' missing in lattice!')
					end if
					if(.not. L%check_exist_bond(actual_name,actual_name2))then
						call L%set_bond(actual_name,actual_name2,actual_ind,actual_ind2)
					end if
				end if
			end do
		end if
...
subroutine draw_l(L,filename,label_bond,fixed)
	...
	call L%draw_tn(filename,'',[(.false.,i=1,L%max_site_num)],label_bond,fixed)

end subroutine


subroutine draw_tn(L,filename,tnname,includes,label_bond_,fixed_)

	class(lattice),intent(in)::L
	character(len=*),intent(in)::filename
	character(len=*),optional,intent(in)::tnname
	logical,intent(in)::includes(:)
	logical,intent(in),optional::label_bond_,fixed_
	logical::label_bond,fixed
...
subroutine contract_type(L,Tout,T1,T2,includes1,includes2)
	...
	if(all(.not.includes1))then
		...
		Tout=T1
	else if(any(includes1 .and. includes2))then
