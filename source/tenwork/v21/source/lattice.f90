...
type lattice
	...
	procedure:: insert_info_whole
	procedure:: insert_info_pos
	procedure:: insert_info_name
	generic,public:: insert_info=>insert_info_whole,insert_info_pos,insert_info_name
	procedure,public:: get_info_whole
	procedure,public:: get_info_pos
	procedure,public:: get_info_name
	generic,public:: get_info=>get_info_whole,get_info_pos,get_info_name
	procedure,public:: set_info_whole
	procedure,public:: set_info_pos
	procedure,public:: set_info_name
	generic,public:: set_info=>set_info_whole,set_info_pos,set_info_name
end type   
...
logical function check_exist_pos(L,pos)
	...
	if(.not. L%empty_tag .and. pos(1)>=1 .and. pos(1)<=L%L1 .and. pos(2)>=1 .and. pos(2)<=L%L2) then
...
subroutine add_from_ten(L,pos,my_name_,my_tensor,save_tag_)
...
subroutine get_info_whole(L,val_name,val)
...
subroutine get_info_name(L,name,val_name,val)
...
subroutine get_info_pos(L,pos,val_name,val)
...
subroutine set_info_whole(L,val_name,val)
...
subroutine set_info_name(L,name,val_name,val)
...
subroutine set_info_pos(L,pos,val_name,val)
...
subroutine insert_info_whole(L,val_name,val)
...
subroutine insert_info_name(L,name,val_name,val)
...
subroutine insert_info_pos(L,pos,val_name,val)
...
subroutine draw_l(L,filename,label_bond,fixed,check_tag)
	...
	call L%draw_tn(filename,'',[(.false.,i=1,L%max_site_num)],[(-1,i=1,L%max_site_num)],label_bond,fixed,check_tag)

end subroutine

subroutine draw_tn(L,filename,tnname,includes,path,label_bond_,fixed_,check_tag_)
use mod_mpi_info
	...
	logical,intent(in),optional::label_bond_,fixed_,check_tag_
	logical::label_bond,fixed,check_tag
