...
subroutine draw_l(L,filename,label_bond,free_style)

	class(lattice),intent(in)::L
	character(len=*),intent(in)::filename
	logical,intent(in),optional::label_bond,free_style
	integer::i

	call L%draw_tn(filename,'',[(.false.,i=1,L%max_site_num)],label_bond,free_style)

end subroutine


subroutine draw_tn(L,filename,tnname,includes,label_bond_,free_style_)

	class(lattice),intent(in)::L
	character(len=*),intent(in)::filename
	character(len=*),optional,intent(in)::tnname
	logical,intent(in)::includes(:)
	logical,intent(in),optional::label_bond_,free_style_
	logical::label_bond,free_style
	...
	if(present(free_style_)) then
		free_style=free_style_
	else
		free_style=.false.
	end if
	...
	call system('rm ./'//trim(filename)//'.png')
	call L%check_consistency()
