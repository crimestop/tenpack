...
subroutine restore_pos(L,pos)
	...
	if(.not. L%sites(rawpos)%back_up_tag) then
		call wc_error_stop('lattice.restore','tensor at '//trim(str(pos))//' has not been backed up before')
	end if
...
subroutine invert_bond_grp(G,T)
	...
	integer::i,j,nb_no
	character(len=max_char_length)::name,nb_name,ind,nb_ind
	...
	do i=1,G%lat%max_site_num
		if(G%lat%sites(i)%exist_tag)then
			name=G%lat%sites(i)%name
			if(G%check_contain(name))then
				do j=1,G%lat%get_nb_num(name)
					call G%lat%get_bond_no(name,j,ind,nb_name,nb_no,nb_ind)
					if(.not.G%check_contain(nb_name)) then
						call T%setName(ind,nb_ind)
					end if
				end do
			end if
		end if
