...
type, extends(tensor):: tn_tensor
	...
	procedure::absorb_except_pos
	procedure::absorb_except_name
	generic,public::absorb_except=>absorb_except_pos,absorb_except_name
...
subroutine absorb_except_pos(T,pos,in_dir)
...
subroutine absorb_except_name(T,name,in_dir)

	class(tn_tensor),intent(inout)::T
	type(lattice),pointer::plat
	character(len=*),intent(in)::name
	character(len=*),optional,intent(in)::in_dir
	integer::L1,L2,m,n,pos(2)
	character(len=2)::dir
	...
	call T%absorb_except(pos,in_dir)
