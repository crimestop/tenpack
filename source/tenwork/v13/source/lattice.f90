...
type group
	...
	procedure::grp_get_size
	generic,public::get_size=>grp_get_size
	procedure,public::get_names

end type  

public lattice,group,lat_absorb_tensor,lat_contract_type
...
logical function check_exist_bond_name(L,name1,name2)
	...
	!write(*,*)trim(name1),'---',trim(name2)
	!write(*,*)L%bonds_num_rawpos(rawpos1,rawpos2)
	!call L%draw('test_check_bond_exist_name',check_tag=.false.)
...
subroutine set_bond_as_cluster(L,L_old,clusters)
	...
	character(len=max_char_length)::ind1,ind2,name1,name2,actual_name1,actual_name2,actual_ind1,actual_ind2,nb_ind_temp
			...
			do j=1,L_old%sites(i)%nb_num
				...
				if(nb_rawpos>i) then
					...
					if(incl1)then
						...
						call belong1%get_bond(actual_name1,no,actual_ind1,name1,nb_no,nb_ind_temp)
					end if
					...
					if(incl2)then
						...
						call belong2%get_bond(actual_name2,no,actual_ind2,name2,nb_no,nb_ind_temp)
					end if
...
subroutine draw_l(L,filename,label_bond,fixed,check_tag)
	...
	call L%draw_tn(filename,'',[(.false.,i=1,L%max_site_num)],label_bond,fixed,check_tag)

end subroutine


subroutine draw_tn(L,filename,tnname,includes,label_bond_,fixed_,check_tag_)
...
subroutine draw_grp(G,filename,tnname,label_bond,fixed,check_tag)
	...
	call G%lat%draw(filename,tnname,G%includes,label_bond,fixed,check_tag)
...
subroutine grp_get_size(G,L1,L2)
